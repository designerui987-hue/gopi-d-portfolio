    ...
    >
      <div className="relative w-full h-full overflow-hidden rounded-2xl">
        {src ? (
          ...
          <motion.div
            style={{ y: reduced ? 0 : y }}
            className="absolute inset-0 grid-bg opacity-[0.14] flex items-center justify-center"
          >
      ...
      <div className="relative mt-20 space-y-36">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="rounded-3xl border border-border/30 bg-surface/20 p-8 space-y-6">
              <div className="flex items-center gap-2 text-accent">
                ...
                <span className="font-mono text-xs font-semibold tracking-wider">01 · ENTERPRISE DESIGN (2025–2026)</span>
              </div>
              <h3 className="font-display text-2xl text-foreground font-light">Internal HRMS Platform</h3>
            ...
            <div className="rounded-3xl border border-border/30 bg-surface/20 p-8 space-y-6">
              <div className="flex items-center gap-2 text-accent">
                ...
                <span className="font-mono text-xs font-semibold tracking-wider">02 · FINTECH PROTOTYPE (2026)</span>
              </div>
              <h3 className="font-display text-2xl text-foreground font-light">Nova Stock Analytics</h3>
              <p className="text-sm text-muted-foreground leading-relaxed font-light">
        ...
        <section className="space-y-12 border-t border-border/20 pt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { t: "Clarity over novelty", d: "Aesthetics must serve layout logic. If a design element doesn't help users complete a task or understand data, it should be removed." },
              { t: "Systems, not screens", d: "Isolated layouts break under pressure. Every component is designed to map directly to shared spacing rules and design system tokens." },
              { t: "Honest simplicity", d: "No flashing animations, no dark patterns, and no layout tricks. Clean hierarchy, high-contrast text, and respect for user focus." },
            ].map((principle, idx) => (
              <motion.div
                key={principle.t}
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.7, delay: idx * 0.1, ease: EASE_EDITORIAL }}
                className="rounded-3xl border border-border/30 bg-surface/10 p-6 space-y-3"
              >
                <div className="font-display text-xl text-foreground font-light">{principle.t}</div>
                <p className="text-xs text-muted-foreground leading-relaxed font-light">{principle.d}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-12 border-t border-border/20 pt-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            <ToolGroupCard n="01" label="DESIGN" tools={["Figma", "FigJam", "Component systems", "Token architecture", "Responsive layout matrices"]} />
            <ToolGroupCard n="02" label="DEVELOPMENT" tools={["TypeScript", "React", "TanStack Router", "Framer Motion", "Tailwind CSS", "Vite"]} />
            <ToolGroupCard n="03" label="RAPID PROTOTYPING" tools={["Lovable", "AI-assisted UI staging", "Figma to code handoff logs"]} />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start pt-8 border-t border-border/20">
          ...
          <div className="lg:col-span-7">
            <div className="space-y-6">
              {[
                { title: "Advanced Design Tokens", desc: "Mapping design variables across theme scopes (light/dark) using strict W3C token standards." },
                { title: "Frontend Performance Specs", desc: "Studying browser rendering paths, CSS transitions, and layout paint shifts to write faster UI code." },
                { title: "WCAG 2.2 Standards", desc: "Deepening my knowledge of screen-reader landmarks, focus locks, and structural markup compliance." },
              ].map((item, idx) => (
                <div key={item.title} className="flex gap-4 items-start border-b border-border/25 pb-4 last:border-b-0">
                  <Compass className="h-5 w-5 text-accent shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-sm font-semibold text-foreground">{item.title}</h4>
                    <p className="text-xs text-muted-foreground font-light leading-relaxed mt-1">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
